...
class ContractValidator:
    ...
    def __init__(
        ...
        if not simulation_mode:
            ...
            try:
                ...
                if not default_account:
                    if self.web3.eth.accounts:
                        ...
                        logger.info(
                            f"Using default account from node: {self.default_account}"
                        )
            ...
            logger.info(
                "Running in simulation mode - no blockchain connection required"
            )

    def _load_contracts(
        self, contract_addresses: Dict[str, str], abi_directory: str
    ) -> None:
        """
    ...
    def add_contract(
        self, name: str, address: str, abi: Union[str, List, Dict]
    ) -> None:
        """
        ...
        try:
            ...
            raise ContractValidationError(
                f"Failed to add contract {name}: {str(e)}"
            ) from e
    ...
    def call_function(
        ...
        **kwargs,
        ...
        if self.simulation_mode:
            ...
            return self._simulate_function_call(
        ...
        if not WEB3_AVAILABLE:
            ...
            raise ContractValidationError(
                "Web3.py is required for contract function calls"
            )
        ...
        try:
            ...
            if not transaction:
                # Read operation (call)
                logger.debug(
                    f"Performing read call to {function_name} with args: {args}"
                )
                ...
                return self._submit_transaction(
                    contract, function_name, args, kwargs, tx_params
                )
