...
class NFTManager:
    ...
    def get_owner(self, nft_contract: str, token_id: int) -> str:
        ...
        try:
            logger.info(
                f"Getting owner of token ID {token_id} in contract {nft_contract}"
            )
    ...
    def get_token_balance(
        ...
        try:
            if token_id is not None:
                ...
                logger.info(
                    f"Getting ERC-1155 balance for token ID {token_id}, account {account}"
                )
                ...
                result = self.contract_validator.call_function(
                    nft_contract, "balanceOf", account
                )
    ...
    def transfer(
        ...
        **kwargs,
        ...
        try:
            if is_erc1155:
                logger.info(
                    f"Transferring ERC-1155 token ID {token_id} to {to_address}"
                )

                # Get the sender address
                from_address = kwargs.get(
                    "from", self.contract_validator.default_account
                )
                if not from_address:
                    raise BlockchainError(
                        "No sender address provided or default account set"
                    )
    ...
    def get_approved(self, nft_contract: str, token_id: int) -> str:
        ...
        try:
            ...
            result = self.contract_validator.call_function(
                nft_contract, "getApproved", token_id
            )
