...
def load_config(config_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    config = {
        ...
        "contracts": {
            "abi_directory": os.environ.get(
                "LLAMA_BLOCKCHAIN_ABI_DIR", DEFAULT_ABI_DIRECTORY
            ),
            "addresses": {},
        },
        "transaction": {
            "timeout": int(
                os.environ.get("LLAMA_BLOCKCHAIN_TX_TIMEOUT", DEFAULT_TIMEOUT)
            ),
            "gas_limit_multiplier": float(
                os.environ.get(
                    "LLAMA_BLOCKCHAIN_GAS_MULTIPLIER", DEFAULT_GAS_LIMIT_MULTIPLIER
                )
            ),
        },
        "provenance": {
        ...
        "zk_proofs": {
            "enabled": os.environ.get("LLAMA_BLOCKCHAIN_ZK_ENABLED", "false").lower()
            == "true"
        },
    }
