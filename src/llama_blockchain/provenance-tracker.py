...
class ProvenanceTracker:
    ...
    def __init__(
        ...
        if mode == "on-chain" and (
            not contract_validator or not provenance_contract_name
        ):
            logger.warning(
    ...
    def track(
        self, data: Any, action: str, metadata: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    def _store_on_chain(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Attempting to store provenance record on-chain: {record['id']}"
            )
    ...
    def get_record(self, provenance_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
                ...
                if result.get("success", False) and result.get("result"):
                    ...
                    if isinstance(contract_result, tuple) and len(contract_result) >= 5:
                        return {
                            ...
                            "metadata": (
                                json.loads(contract_result[3])
                                if contract_result[3]
                                else {}
                            ),
                        }
                    else:
                        logger.warning(
                            f"Unexpected contract result format: {contract_result}"
                        )
                        return None
                else:
            ...
            raise ProvenanceError(
                f"Failed to retrieve provenance record: {str(e)}"
            ) from e
    ...
    def get_history(
        ...
        try:
            ...
            raise ProvenanceError(
                f"Failed to retrieve provenance history: {str(e)}"
            ) from e
